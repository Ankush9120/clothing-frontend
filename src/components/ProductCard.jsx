      ...
      <div className="relative">
        <Link to={`/product/${data?.id}`}>
          <motion.div className="h-min select-none grid gap-[9px]" whileTap={{ scale: 0.97 }} transition={{ type: "spring", stiffness: 400, damping: 17 }}>
            <img src={data.image} className="aspect-auto object-contain rounded-[8px]" alt={data.name} />
          </motion.div>
        </Link>
        <LikeIcon product={data} className="absolute bottom-2 right-2 backdrop-blur-[20px]" />
      </div>
      <div className="p-0">
        <span className="text-[10px] leading-[14px] font-semibold">{data.name}</span>
        <p className="line-clamp-2 text-[10px] leading-[12px] font-albert">{data.description}</p>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="text-green-100 text-[12px] leading-[16px]">{data.discount || 80}% off</div>
        </div>
      </div>
    </div>
  );
